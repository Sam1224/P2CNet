...
class FuseNetMPSatTrainer(BaseFuseNetTrainer):
    def __init__(self, models, optimizers, loss, metrics, resume, config,
        ...
        super(FuseNetMPSatTrainer, self).__init__(
            models,
            optimizers,
            loss=loss,
            metrics=metrics,
        ...
        lambda_1 = config["trainer"]["lambda_1"]
        print("Weight of MP Loss: {}".format(lambda_1))
        if lambda_1:
    ...
    def _train_sat_epoch(self, epoch):
        ...
        for batch_idx, (sats, maps_partial, maps_complete, _) in enumerate(self.train_data_loader):
            ...
            # BCE Dice Loss
            loss_sum_bce_dice, bce_loss, dice_loss = self.loss["BCE_Dice"](maps, maps_complete)

            # Missing Part Loss
            mp_loss = self.lambda_1 * self.loss["MP"](maps, maps_partial, maps_complete)

            loss_sum = loss_sum_bce_dice + mp_loss
            loss_sum.backward()
            self.optimizer.step()
            ...
            self.writer.add_scalar('sat_mp_loss', mp_loss.item())
            total_loss += loss_sum.item()
            total_bce_loss += bce_loss.item()
            total_dice_loss += dice_loss.item()
            total_mp_loss += mp_loss.item()

            if self.verbosity >= 2 and batch_idx % self.log_step == 0:
                self.logger.info(
                    'Train Epoch: {} [{}/{} ({:.0f}%)] '
                    'Total:{:.6f} '
                    'FuseSat: [BCE:{:.6f}, Dice:{:.6f}, MP:{:.6f}]'.format(
                        epoch,
        ...
        log = {
            ...
            'sat_mp_loss': total_mp_loss / len(self.train_data_loader)
        }
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (sats, maps_partial, maps_complete, _) in enumerate(self.train_data_loader):
            ...
            sat_mp_loss = self.lambda_1 * self.loss["MP"](sat_maps, maps_partial, maps_complete)

            inputs = maps_partial
            par_maps, _ = self.par(inputs, encs, fms)
            par_loss_sum, par_bce_loss, par_dice_loss = self.loss["BCE_Dice"](par_maps, maps_complete)

            loss_sum = sat_loss_sum + par_loss_sum + sat_mp_loss
            del inputs, encs, fms
            torch.cuda.empty_cache()
            loss_sum.backward()
            self.optimizer.step()
            ...
            self.writer.add_scalar('sat_loss', sat_loss_sum.item() + sat_mp_loss.item())
            self.writer.add_scalar('sat_bce_loss', sat_bce_loss.item())
            self.writer.add_scalar('sat_dice_loss', sat_dice_loss.item())
            self.writer.add_scalar('sat_mp_loss', sat_mp_loss.item())
            ...
            total_loss += loss_sum.item()
            total_sat_loss += sat_loss_sum.item() + sat_mp_loss.item()
            total_sat_bce_loss += sat_bce_loss.item()
            total_sat_dice_loss += sat_dice_loss.item()
            total_sat_mp_loss += sat_mp_loss.item()
        ...
        log = {
            ...
            'sat_mp_loss': total_sat_mp_loss / len(self.train_data_loader),
        ...
        if self.do_validation:
            ...
            if self.lr_scheduler is not None:
                self.lr_scheduler.step(val_log["val_metrics_mIoU"])
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for batch_idx, (sats, maps_partial, maps_complete, _) in enumerate(self.valid_data_loader):
                ...
                self.writer.set_step((epoch - 1) * len(self.train_data_loader) + batch_idx)
                self.writer.add_scalar('val_total_loss', loss_sum.item())
                self.writer.add_scalar('val_sat_loss', sat_loss_sum.item() + sat_mp_loss.item())
                self.writer.add_scalar('val_sat_bce_loss', sat_bce_loss.item())
                self.writer.add_scalar('val_sat_dice_loss', sat_dice_loss.item())
                self.writer.add_scalar('val_sat_mp_loss', sat_mp_loss.item())
        ...
        return {
            ...
            'val_sat_mp_loss': total_sat_mp_loss / len(self.train_data_loader),
