...
class BasicTrainer(BaseNetTrainer):
    ...
    def _train_net_epoch(self, epoch):
        ...
        total_bce_loss = 0.
        total_dice_loss = 0.
        for batch_idx, (sats, _, maps_complete, _) in enumerate(self.train_data_loader):
            sats, maps_complete = sats.to(self.device), maps_complete.to(self.device)
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (sats, _, maps_complete, _) in enumerate(self.train_data_loader):
            ...
            if self.verbosity >= 2 and batch_idx % self.log_step == 0:
                ...
                self.save_images(sats, maps, maps_complete, epoch=epoch, batch_idx=batch_idx, r=2, c=3)
    ...
    def _valid_epoch(self, epoch):
        ...
        self.metrics.reset()
        with torch.no_grad():
            for batch_idx, (sats, _, maps_complete, _) in enumerate(self.valid_data_loader):
                sats, maps_complete = sats.to(self.device), maps_complete.to(self.device)
                inputs = sats
    ...
    def save_images(self, sats, maps, maps_complete, epoch, batch_idx, r=1, c=3):
        bs = sats.size(0)
        r = r if bs >= r else bs
        sats = sats.cpu().numpy()
        ...
        if r == 1:
            ...
            axs[1].imshow(maps[0], cmap='gray')
            axs[1].axis('off')

            axs[2].set_title('Complete Map')
            axs[2].imshow(maps_complete[0], cmap='gray')
            axs[2].axis('off')
        else:
            count = 0
            for row in range(r):
                ...
                axs[row, 1].set_title('Generated Map - {}'.format(count))
                axs[row, 1].imshow(maps[count], cmap='gray')
                axs[row, 1].axis('off')

                axs[row, 2].set_title('Complete Map - {}'.format(count))
                axs[row, 2].imshow(maps_complete[count], cmap='gray')
                axs[row, 2].axis('off')
                count += 1
