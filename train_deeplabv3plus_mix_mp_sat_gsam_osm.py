...
def main(config, config_log, resume):
    ...
    if config["trainer"]["pretrained_sat"] and config["trainer"]["pretrained_par"]:
        print("load pretrained sat branch from {}".format(config["trainer"]["pretrained_sat"]))
        print("load pretrained par branch from {}".format(config["trainer"]["pretrained_par"]))
        ...
        checkpoint = torch.load(config["trainer"]["pretrained_par"], map_location="cuda:1")
    ...
    elif config["trainer"]["pretrained_sat"]:
        print("load pretrained sat branch from {}".format(config["trainer"]["pretrained_sat"]))
        checkpoint = torch.load(config["trainer"]["pretrained_sat"])
    ...
    optimizers = {}
    # params = [
    #     {"params": [p for n, p in sat.named_parameters() if p.requires_grad],
    #      "lr": config["optimizer"]["args"]["lr"] / 10},
    #     {"params": [p for n, p in par.named_parameters() if p.requires_grad and "adaptor" not in n],
    #      "lr": config["optimizer"]["args"]["lr"] / 10},
    #     {"params": [p for n, p in par.named_parameters() if p.requires_grad and "adaptor" in n],
