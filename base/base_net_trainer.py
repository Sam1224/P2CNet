...
class BaseNetTrainer:
    ...
    def pre_train(self):
        ...
        :return:
        """
        if not os.path.exists(self.config["trainer"]["pretrain_net_path"]):
            print("pre train net")
            self.pre_train_net()
            self._save_net_checkpoint()
        else:
            print("load net")
            checkpoint = torch.load(self.config["trainer"]["pretrain_net_path"], map_location='cpu')
    ...
    def pre_train_net(self):
        """
        Pre training logic
        """
        for epoch in range(self.config["trainer"]["pretrain_net_epochs"]):

            # save logged informations into log dict
            log = {'pretrain net epoch': epoch}
            result = self._train_net_epoch(epoch)
            for key, value in result.items():
                log[key] = value
    ...
    def _save_net_checkpoint(self):
        """
        Saving pre-trained checkpoints
        """
        arch = type(self.model).__name__
        state = {
            'arch': arch,
        ...
        filename = os.path.join(self.checkpoint_dir, 'pretrain_net.pth')
        torch.save(state, filename)
        self.logger.info("Saving net checkpoint: {} ...".format(filename))
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        if checkpoint['config']['gen_arch'] != self.config['gen_arch']:
            self.logger.warning(
        ...
        if checkpoint['config']['gen_optimizer']['type'] != self.config['gen_optimizer']['type']:
